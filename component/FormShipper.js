...
// import { FILL_ADDRESS_SHIPPER, FILL_EMAIL_CONFIRM_SHIPPER, FILL_EMAIL_SHIPPER, FILL_GENDER_SHIPPER, FILL_NAME_SHIPPER, FILL_PHONE_SHIPPER, SET_DATA_SHIPPER, UPDATE_FIELD_SHIPPER } from "../reducer/customerReducer";
...
export default function FormShipper({ status }) {
    ...
    useEffect(() => {
        ...
        // }
    }, [name_shipper.error, phone_shipper.error, email_shipper.error])

    useEffect(() => {
        value_listener('name_shipper', name.value)
    }, [name.value])
    useEffect(() => {
        value_listener('phone_shipper', phone.value)
    }, [phone.value])
    useEffect(() => {
        value_listener('email_shipper', email.value)
    }, [email.value])
